          ...
          {cart.map((item) => (
            <div className={style.cartBody} key={item._id}>
              <div className={style.cartImage}>
                <Image src={item.ImageURL} height={90} width={65} alt={`product ${item._id} image`} /> {/* Use the Image component */}
              </div>
              <p>{item.Name}</p>
              <p>$ {item.Price}</p>
              <p>{item.quantity}</p>
              <div className={style.cartButtons}>
                ...
                <button onClick={() => dispatch(removeFromCart(item._id))} className={style.remove}>
                  x
                </button>
              </div>
